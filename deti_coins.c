...
//
// saving and reporting DETI coins
//
...
int main(int argc, char **argv)
{
  ...
  if ((argc >= 2 && argc <= 4) && argv[1][0] == '-' && argv[1][1] == 's')
  {
    ...
    seconds = (argc > 2) ? atoi(argv[2]) : 1800; // Usa atoi para converter o argumento
    if (seconds <= 0)
    {
      fprintf(stderr, "main: invalid number of seconds. It must be a positive integer.\n");
      exit(1);
    }
    if (seconds < 120)
      seconds = 120; // Pelo menos 2 minutos
    if (seconds > 7200)
      seconds = 7200; // No máximo 2 horas
