...
#ifndef DETI_COINS_CLIENT_AVX2_H
#define DETI_COINS_CLIENT_AVX2_H
...
static void handle_received_coins(uint32_t (*coins)[13], uint32_t num_coins) {
    for (uint32_t i = 0; i < num_coins; i++) {
        ...
        for (int j = 0; j < 13; j++) {
            ...
            // Extract bytes in correct order (little-endian)
            coin_text[pos++] = (char)((word >> 0) & 0xFF);
            coin_text[pos++] = (char)((word >> 8) & 0xFF);
            coin_text[pos++] = (char)((word >> 16) & 0xFF);
            coin_text[pos++] = (char)((word >> 24) & 0xFF);
        }
        coin_text[pos] = '\0';
        printf("%s\n", coin_text);
...
#endif // DETI_COINS_CLIENT_AVX2_H
